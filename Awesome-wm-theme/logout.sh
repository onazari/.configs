...
ACTION=`zenity --width=90 --height=200 --list --radiolist --text="Select logout action" --title="Logout" --column "Choice" --column "Action" TRUE Hibernate FALSE Reboot FALSE LockScreen FALSE Suspend`

if [ -n "${ACTION}" ];then
  case $ACTION in
  Hibernate)
    zenity --question --text "Are you sure you want to halt?" && sudo /usr/sbin/pm-hibernate
    ## or via ConsoleKit
